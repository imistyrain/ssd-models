...
//const char* classNames[] = { "background","face", "sofa", "chair","table", "lamp"};
...
"{ proto          | ../Face/MobileNetSSD_deploy.prototxt | model configuration }"
"{ model          | ../Face/MobileNetSSD_face.caffemodel | model weights }"
...
int main(int argc, char** argv)
{
	...
	string test_dir = "imgs";
	string result_dir = "results";
	auto files = getAllFilesinDir(test_dir);
	for (;;)
//	for(int i=0;i<files.size();i++)
	{
 		Mat frame;
 		cap >> frame; // get a new frame from camera/video or read image
//		frame = cv::imread(test_dir+"/"+files[i]);
		if (frame.empty())
		{
			waitKey();
			break;
...
// 		string result_path = result_dir + "/" + files[i];
